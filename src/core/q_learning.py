...
class QN(object):
    ...
    def get_action(self, state):
        ...
        Args:
            state: observation from gym
        """
        if np.random.random() < self.config.soft_epsilon and not self.config.diversification:
    ...
    def train(self, exp_schedule, lr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            while True:
                ...
                idx, q_input = replay_buffer.store_frame(state)
                ...
                # make sure pacman stays on the same map
                if (new_state[...,9:] == 0).all(): done = True
                
                # cheak if the agent was killed
                ...
                replay_buffer.store_effect(idx, action, reward, done, dead)
                state = new_state
...
#            print(i)
    ...
    def record(self):
        ...
        env = PreproWrapper(env, prepro=my_prepro, shape=(40, 40, self.config.nb_pawns+1), 
    ...
    def run_no_train(self, exp_schedule, lr_schedule):
        # initialize
        self.initialize()
