...
class PreproWrapper(gym.Wrapper):
    ...
    def __init__(self, env, prepro, shape, overwrite_render=True, high=255):
        ...
        with open ('utils/pellet_locations', 'rb') as fp:
            self.pellet_list = pickle.load(fp)
    ...
    def step(self, action):
        ...
        """
        obs, reward, done, info = self.env.step(action)
        self.obs = self.prepro(obs, pellet_list = self.pellet_list)
        return self.obs, reward, done, info


    def reset(self):
        self.obs = self.prepro(self.env.reset(), pellet_list = self.pellet_list)
        return self.obs
